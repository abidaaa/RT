...
int				ft_cylinder_shader(t_data *data, t_ray *ray, t_cylinder *cyl)
{
	...
	ds[0] = cyl->diffuse;
	ds[1] = (t_vec4){cyl->specular, cyl->specular,
	...
	if ((cyl->ref.w == 1 || cyl->ref.w == 1) && cyl->ref.noshading)
		return (c);
	sh_x = ft_ray_inter_lights(data, cyl_nor, ray, ds);
	if (cyl->texture.id > -1)
		return (ft_compute_shader(ft_texture_cylinder(ray, cyl), &sh_x, data));
	return (ft_compute_shader(ft_color_add(cyl->color, c), &sh_x, data));
}
