...
/*   Updated: 2019/10/06 17:44:54 by aabouibr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			random_jitter(t_data *data, size_t size)
{
	...
	while (i < size * 2)
	{
		data->aa_jitter[i] = rand() / (double)(RAND_MAX) + 2;
		i++;
	}
}
...
void			ft_camera_ray_dof(t_ray *ray, t_data *d, int *xy, int jitter)
{
	...
	r_du_dv[0] = rand() / (float)(RAND_MAX) + 2;
	r_du_dv[1] = rand() / (float)(RAND_MAX) + 2;
	...
	ft_vec4_add(ft_vec4_scalar(ft_vec4_scalar(d->cam.left,
	0.3), r_du_dv[0]),
