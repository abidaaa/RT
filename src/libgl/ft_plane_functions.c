...
/*   Updated: 2019/10/06 16:29:59 by aabouibr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_plane_shader(t_data *data, t_ray *ray, t_plane *pl)
{
	...
	ds[0] = pl->diffuse;
	ds[1] = (t_vec4){pl->specular, pl->specular,
	...
	if ((pl->ref.w == 2 || pl->ref.w == 1) && pl->ref.noshading)
		return (c);
	sh_x = ft_ray_inter_lights(data, pl_nor, ray, ds);
	if (pl->texture.id > -1)
		return (ft_compute_shader(ft_texture_plane(ray, pl), &sh_x, data));
